...
import { z } from 'zod'
import { IProcedureOrder, useCreateProcedureOrder, useUpdateProcedureOrder } from './services'
...
interface Props {
  item: IProcedureOrder
  onClose: () => void
}

export const ProcedureOrder: FC<Props> = ({ item, onClose }) => {
  const {
    control,
    register,
    handleSubmit,
    formState: { errors },
  } = useForm<IProcedureOrder>({
    defaultValues: item.id ? item : undefined,
    resolver: zodResolver(schema),
  })
  const create = useCreateProcedureOrder()
  const update = useUpdateProcedureOrder()

  const orders = useFetchOrders().data ?? []

  const handleCreate = (data: IProcedureOrder) => {
    create.mutateAsync(data).then(() => {
      onClose()
    })
  }

  const handleUpdate = (data: IProcedureOrder) => {
